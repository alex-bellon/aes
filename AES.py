...
def make_matrix_list(byte_string, decrypt):
    arr = []
        ...
        for i in range(0,4):
            for j in range(0,4):
                if loop.count >= len(byte_string): #we've encoded all the chars
                    return False
                matrix[j][i] = byte_string[loop.count] #add in value, in col major order
                loop.count += 1
        return True

    loop.count = count;
    while(loop()): #fill in matrices
        arr.append(matrix)
...
def mix_columns(m):
    A = [[2, 3, 1, 1],
         ...
         [3, 1, 1, 2]]
    for i in range(0,4):
        v = [m[0][i], m[1][i], m[2][i], m[3][i]]
        r = matrix_mul(A, v)
        m[0][i], m[1][i], m[2][i], m[3][i] = r[0], r[1], r[2], r[3]
...
def key_schedule(key,n,nk):
    ...
    for i in range(0, 4*nk):
        ...
        if(i < n):
            ...
            curr_word = struct.pack(">I", getword(curr_key[(i-n)*4:(i-n+1)*4]) ^ rotword(subword(last_word)) ^ rcon[i//n - 1]<<24)
        elif(i >= n and n > 6 and i%n==4):
            curr_word = struct.pack(">I", getword(curr_key[(i-n)*4:(i-n+1)*4]) ^ subword(last_word))
        else:
            curr_word = struct.pack(">I", getword(curr_key[(i-n)*4:(i-n+1)*4]) ^ last_word)
        curr_key += curr_word
        last_word = getword(curr_word)
    for i in range(0,16*nk,16):
...
def print_matrix(matrix):
    for i in range(0, len(matrix)):
        ...
        for j in range(0, len(matrix[0])):
            print('%x ' % matrix[i][j], end='')
        print(']')
    print()

def do_rounds(matrix, r, keys):
    add_key(matrix, keys[0])
    for i in range(1, r+1):
        sub_bytes(matrix)
        shift_rows(matrix)
        if(i != r):
            mix_columns(matrix)
        add_key(matrix, keys[i])

def inv_do_rounds(matrix, r, keys):
    for i in range(r, 0, -1):
        inv_add_key(matrix, keys[i])
        if(i != r):
...
def get_output(li):
    ...
    for matrix in li:
        for i in range(0,4):
            for j in range(0,4):
                byte_string += struct.pack("B", matrix[j][i])
    return byte_string
...
#setting rounds
n = 8 if options.keysize == '256' else 4
r = 14 if options.keysize == '256' else 10
nk = 15 if options.keysize == '256' else 11
...
key_bytes = key_schedule(key_bytes, n, nk)
...
if(options.mode == 'decrypt'):
    num = output[len(output)-1]
    output = output[0: len(output)-num]
o = open(options.outputfile, 'wb')
o.write(output)
